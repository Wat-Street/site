...
export default function StockCryptoSearch({
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (search) {
        ...
        // clear the state
        setTopFiveStockResults([]);
        setTopFiveCryptopResults([]);
      }
    }, 300);
  ...
  const [topFiveStockResults, setTopFiveStockResults] = useState<
    ItemStockProps[]
  >([]);
  const [topFiveCryptoResults, setTopFiveCryptopResults] = useState<
    ItemCryptoProps[]
  >([]);
    ...
    if (isStock) {
      try {
        const response = await fetch(
          `https://api.polygon.io/v3/reference/tickers?market=stocks&search=${search}&active=true&limit=5&apiKey=${process.env.NEXT_PUBLIC_POLYGON_API_KEY}`
        );
      ...
      try {
        ...
        const response = await fetch(
          `https://api.polygon.io/v3/reference/tickers?market=crypto&search=${search}&active=true&apiKey=${process.env.NEXT_PUBLIC_POLYGON_API_KEY}`
        );
        const data = await response.json();
        console.log(data);
...
const StockItem: React.FC<ItemStockProps> = ({
  ticker,
  name,
  primaryExchange,
  setOpen,
  ...
  return (
    <button
      className="relative w-full flex justify-between cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent hover:text-accent-foreground ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-ring"
...
const CryptoItem: React.FC<ItemCryptoProps> = ({
  ticker,
  name,
  currencySymbol,
  setOpen,
